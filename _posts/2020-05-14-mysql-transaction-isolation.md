...
1) 가장 단순하고 가장 엄격한 격리수준으로 가장 강력한 LEVEL이다.
2) SELECT 문에 사용하는 모든 테이블에 shared lock이 발생한다.
3) InnoDB에서 기본적으로 순수한 SELECT 작업은 아무런 잠금을 걸지않고 동작하는데, 격리수준이 SERIALIZABLE일 경우 읽기 작업에도 공유 잠금을 설정하게 되고, 이러면 동시에 다른 트랜잭션에서 이 레코드를 변경하지 못하게 된다.
4) 이러한 특성 때문에 동시처리 능력이 다른 격리수준보다 떨어지고, 성능저하가 발생하게 된다.
...
그러므로 위의 UPDATE 구문은 레코드에 대해 쓰기 잠금을 시도하려고 하지만 name = younsik1인 레코드는 존재하지 않으므로,
0 row(s) affected가 출력되고, 아무 변경도 일어나지 않게 된다.
그러므로 최종적으로 결과는 name = younsik2가 된다.

#### insert 부정합

~~~ sql
    START TRANSACTION; -- transaction id : 1
    SELECT * FROM User; -- 0건 조회

        START TRANSACTION; -- transaction id : 2
        INSERT INTO User VALUES(1,'younsik1',32);
        COMMIT;

    SELECT * FROM User; -- 여전히 0건 조회
    UPDATE User SET name = 'younsik2' WHERE id = 1; -- 1 row(s) affected
    SELECT * FROM User; -- 1건 조회
    COMMIT;
~~~
...
~~~ sql
    ...
    SELECT * FROM User; -- 여전히 1건 조회 
    UPDATE User SET name = 'younsik2' WHERE id = 1; -- 0 row(s) affected
    SELECT * FROM User; -- 여전히 1건 조회 
    COMMIT;
~~~

반면, DELETE에서는 적용되지 않는다.
